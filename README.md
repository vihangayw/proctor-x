...
- creates `dist/Proctor-X-1.0.3-arm64.dmg` using the custom mount-and-copy DMG builder

This generates:

```text
dist/
 ├── Proctor-X-1.0.3-arm64.dmg
...
```sh
spctl -a -vv "/Volumes/Proctor-X 1.0.3-arm64/Proctor-X.app"
```
...
- The mounted DMG volume name in this project is `Proctor-X 1.0.3-arm64`, so the verification path should match that
  exact volume name.
