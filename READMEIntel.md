...
- creates `dist/Proctor-X-1.0.1.dmg` using the custom mount-and-copy DMG builder
...
```sh
codesign -dv --verbose=4 dist/mac/Proctor-X.app
```
...
```sh
spctl -a -vv dist/mac/Proctor-X.app
```
...
```sh
spctl -a -vv "/Volumes/Proctor-X 1.0.1/Proctor-X.app"
```
...
- The mounted DMG volume name in this project is `Proctor-X 1.0.1`, so the verification path should match that
  exact volume name.
